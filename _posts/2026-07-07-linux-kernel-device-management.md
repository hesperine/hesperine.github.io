...
这一篇记录设备管理的几个基础问题：设备控制器、I/O 控制方式、设备驱动、通用块层、存储系统 I/O 分层，以及“键盘敲入 A 字母”这条路径。Linux 落点放在 MMIO/PIO、DMA、IRQ、driver model、字符设备、块设备、block layer、workqueue。
...
Linux 落点：用户态常通过设备文件访问设备，例如 `/dev/sda`、`/dev/null`、`/dev/tty`。设备文件把 VFS 接口接到驱动的 file operations 上。字符设备和块设备在内核里的对象、路径和调度方式不同。
...
Linux 落点：高性能设备路径经常组合 DMA、中断、软中断、NAPI、workqueue。硬中断路径要短，能延后的工作放到 softirq、threaded IRQ 或 workqueue。
