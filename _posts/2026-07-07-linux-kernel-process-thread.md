---
title: Linux 内核学习笔记：进程与线程
date: 2026-07-07 17:35:00 +0800
categories: [学习笔记, OS与Linux内核笔记]
tags: [Linux, Linux Kernel, Operating System, Process, Thread]
---

这一篇记录进程与线程的几个基础问题：进程基础、线程、调度入口、进程间通信、多线程同步、死锁、锁类型。这一块内容很宽，本文先整理主线；Linux 细节集中落到 `task_struct`、地址空间、文件表、调度状态、IPC 对象、futex 和等待队列。
...
调度时机、调度原则和调度算法放到下一篇“调度算法”处理；本文只记进程/线程和调度器的接口。
...
进程地址空间隔离后，需要 IPC 交换数据或事件。这里按管道、消息队列、共享内存、信号量、信号、Socket 记录。
...
互斥锁、自旋锁、读写锁、乐观锁、悲观锁按等待方式和适用条件整理。
