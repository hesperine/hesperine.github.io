...
| 层次 | 本章对应内容 |
| --- | --- |
| 小林 Code / 八股 | 多线程冲突、死锁、悲观锁/乐观锁、线程数量上限 |
| OSTEP / 教材 | 锁、条件变量、信号量、并发 bug、调度交错 |
| Linux 实现 | futex、wait queue、spinlock、mutex、rwsem、memory barrier、RCU |
...
futex 在内核里按用户地址组织等待队列。用户态的那个整数仍然是锁状态的核心。
...
RCU 适合读多写少的数据结构。它把同步成本从读侧转移到写侧和延迟释放上。
...
| 中断上下文为什么不能睡眠？ | 中断上下文是硬件事件处理路径，没有普通可阻塞 task 的执行流语义；睡眠需要调度器以后恢复当前上下文，中断上下文不满足这种语义。 |
