...
这一篇记录 Linux 命令里的几类入口：网络性能指标、网络配置、socket 信息、吞吐率和 PPS、连通性和延迟、日志 PV/UV 分析。结合这组 OS/Linux 笔记，命令部分扩成各模块的观测入口：进程、内存、文件、设备、网络、内核事件。
...
日志分析适合作为文本处理入口。大日志先看大小和格式，再做流式处理。
