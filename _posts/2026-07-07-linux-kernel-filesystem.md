---
title: Linux 内核学习笔记：文件系统阅读批注
date: 2026-07-07 17:55:00 +0800
categories: [学习笔记, OS与Linux内核笔记]
tags: [Linux, Linux Kernel, Operating System, Filesystem, VFS]
---

这一篇按小林 Code《图解系统》第六章“文件系统”的顺序记：文件系统基本组成、VFS、文件使用、文件存储、空闲空间管理、文件系统结构、目录、软硬链接、文件 I/O。Linux 批注落到 fd、`struct file`、dentry、inode、superblock、page cache、writeback。
...
Linux 批注：现代文件系统实现更复杂，例如 extent、日志、延迟分配、写时复制、校验等机制。学习 VFS 时先掌握 inode/dentry/superblock/page cache；深入具体文件系统时再看 ext4、XFS、btrfs 的磁盘布局。
