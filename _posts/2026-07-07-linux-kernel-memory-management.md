...
这一篇记录内存管理的几个基础问题：虚拟内存、分段、分页、多级页表、TLB、段页式内存管理、Linux 内存管理。基础概念和常见问法先整理清楚，再补 OSTEP 的机制抽象，以及 Linux 里的真实对象、源码入口和可观察现象。
...
Linux 落点：现代 x86-64 Linux 对普通用户进程主要使用平坦分段模型。分段机制仍有架构历史和少量用途，但内存隔离、权限和映射主要依赖分页。学习 Linux 内存管理时，重点放在 VMA 和页表。
